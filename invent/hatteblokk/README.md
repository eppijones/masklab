...
Then open <http://localhost:5273>. Build with `vite build --config invent/hatteblokk/vite.config.ts`
(output lands in `invent/dist/`). Type-check with `npx tsc -p invent/hatteblokk/tsconfig.json --noEmit`.
...
only in `invent/hatteblokk/scripts/*`, which run under the parent's `tsx`. Leaving `server.fs.allow` at
